 ...
 */
 ...
 module sasic_top (
	...
	wire [7:0] A;
	assign A = {in_7, in_6, in_5, in_4, in_3, in_2, in_1, in_0};
    assign {out_7, out_6, out_5, out_4, out_3, out_2, out_1, out_0} = 8'h00;
	assign {oeb_7, oeb_6, oeb_5, oeb_4, oeb_3, oeb_2, oeb_1, oeb_0} = 8'hFF;

    assign {out_15, out_14, out_13, out_12, out_11, out_10, out_9, out_8} = 8'b0;
    assign {oeb_15, oeb_14, oeb_13, oeb_12, oeb_11, oeb_10, oeb_9, oeb_8} = 8'hFF;
    
    wire [7:0] DI = {in_23, in_22, in_21, in_20, in_19, in_18, in_17, in_16};
    assign {oeb_23, oeb_22, oeb_21, oeb_20, oeb_19, oeb_18, oeb_17, oeb_16} = 8'hFF;
    assign {out_23, out_22, out_21, out_20, out_19, out_18, out_17, out_16} = 8'h00;
    
    wire [7:0] DO;
    assign {out_31, out_30, out_29, out_28, out_27, out_26, out_25, out_24} = DO;
	assign {oeb_31, oeb_30, oeb_29, oeb_28, oeb_27, oeb_26, oeb_25, oeb_24} = 8'h00;

	assign oeb_32 = 1;
	assign oeb_33 = 1;
    assign {out_33, out_32} = 2'h0;
	assign {out_39, out_38, out_37, out_36, out_35, out_34} = 6'h00;
    assign {oeb_39, oeb_38, oeb_37, oeb_36, oeb_35, oeb_34} = 6'h3F; 

    aes_128_csr_wrapper AES128 (
        // CSR Lite Bus Interface
        .clk(clk),
        .rst_n(rst_n),
        .cs(in_32),       // Chip Select
        .we(in_33),       // Write Enable
        .addr(A),
        .wdata(DI),
        .rdata(DO),
...
// Wrapper module with an 8-bit CSR-lite interface for a pipelined/multi-cycle AES core.
module aes_128_csr_wrapper (
    // CSR Lite Bus Interface
    input           clk,
    input           rst_n,
    ...
    // Parameters
    //================================================================
    // Define how many clock cycles the AES operation takes
    parameter OPERATION_CYCLES = 10;
    ...
    localparam ADDR_STATE_BASE = 8'h10;
    localparam ADDR_KEY_BASE   = 8'h20;
    localparam ADDR_OUT_BASE   = 8'h30;

    //================================================================
    // Internal Registers and Wires
    //================================================================
    // Registers for DUT inputs and output
    reg [127:0] state_reg;
    reg [127:0] key_reg;
    wire [127:0] out_wire; // Output from the core
    reg [127:0] out_reg;  // Latched output

    // Control and Status logic
    reg         start_pulse;
    reg         done_clear_pulse;
    reg         busy_reg;
    reg         done_reg;
    reg         irq_enable_reg;
    reg [$clog2(OPERATION_CYCLES):0] cycle_counter;

    // Decode write/read strobes
    wire write_strobe = cs & we;
    wire read_strobe  = cs & ~we;
    
    // Conditions for automatically clearing the 'done' flag
    wire is_state_key_write = write_strobe && ((addr >= ADDR_STATE_BASE && addr < ADDR_STATE_BASE + 16) || 
                                               (addr >= ADDR_KEY_BASE   && addr < ADDR_KEY_BASE   + 16));
    wire is_out_read        = read_strobe && (addr >= ADDR_OUT_BASE && addr < ADDR_OUT_BASE + 16);


    //================================================================
    // DUT (Device Under Test) Instantiation
    //================================================================
    // The AES core's inputs are held stable while the operation is busy.
    aes_128 dut (
        .clk(clk),
        .state(state_reg),
        .key(key_reg),
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Pulses are de-asserted after one cycle
            start_pulse    <= 1'b0;
            done_clear_pulse <= 1'b0;
            ...
            if (write_strobe) begin
                case (addr)
                    ADDR_CTRL: begin
                        ...
                        if (wdata[2]) done_clear_pulse <= 1'b1;
                        irq_enable_reg <= wdata[1];
                    end
                    // Map 16 byte writes to the 128-bit state register
                    ADDR_STATE_BASE : state_reg[7:0] <= wdata;
                    ADDR_STATE_BASE+1 : state_reg[15:8] <= wdata;
                    // ... (remaining bytes omitted for brevity, full mapping implied)
                    ADDR_STATE_BASE+15: state_reg[127:120] <= wdata;
                    
                    // Map 16 byte writes to the 128-bit key register
                    ADDR_KEY_BASE : key_reg[7:0] <= wdata;
                    ADDR_KEY_BASE+1 : key_reg[15:8] <= wdata;
                    // ... (remaining bytes omitted for brevity, full mapping implied)
                    ADDR_KEY_BASE+15: key_reg[127:120] <= wdata;
                endcase
            end
        end
    end
    ...
    //================================================================
    // Control, Status, and Counter Logic
    //================================================================
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            busy_reg      <= 1'b0;
            ...
            if (start_pulse) begin
                ...
                if (cycle_counter == OPERATION_CYCLES - 1) begin
                    busy_reg <= 1'b0;
                    out_reg <= out_wire; // Latch the result at the end of the operation
                end
                cycle_counter <= cycle_counter + 1;
            end

            // --- 'done' flag logic ---
            // Set condition: operation just finished
            if (busy_reg && (cycle_counter == OPERATION_CYCLES - 1)) begin
                done_reg <= 1'b1;
            // Clear conditions: new operation starts, manual clear, or implicit clear on read/write
            end else if (start_pulse || done_clear_pulse || is_state_key_write || is_out_read) begin
                done_reg <= 1'b0;
            end
        end
    end
    
    // IRQ is asserted only when 'done' is high AND interrupts are enabled
    ...
    //================================================================
    always @(*) begin
        rdata = 8'h00; // Default output
        if (read_strobe) begin
            case (addr)
                ADDR_STATUS: begin
                    rdata = {6'b0, done_reg, busy_reg};
                end
                default: begin
                    if (addr >= ADDR_OUT_BASE && addr < (ADDR_OUT_BASE + 16)) begin
                        // Select the correct byte from the latched output register
                        rdata = out_reg >> ((addr - ADDR_OUT_BASE) * 8);
                    end
                end
            endcase
        end
